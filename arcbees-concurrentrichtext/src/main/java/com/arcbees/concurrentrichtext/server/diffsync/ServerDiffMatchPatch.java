...
import static com.arcbees.concurrentrichtext.shared.diffsync.Diff.Operation;
...
public class ServerDiffMatchPatch implements DiffMatchPatch {
    ...
    protected LinkedList<Diff> diff_bisect(String text1, String text2, long deadline) {
        ...
        for (int d = 0; d < max_d; d++) {
            ...
            for (int k2 = -d + k2start; k2 <= d - k2end; k2 += 2) {
                ...
                int y2 = x2 - k2;
                while (x2 < text1_length && y2 < text2_length && text1.charAt(text1_length - x2 - 1) == text2.charAt(
                        text2_length - y2 - 1)) {
                    x2++;
                    y2++;
                }
    ...
    public void diff_cleanupEfficiency(LinkedList<Diff> diffs) {
        ...
        while (thisDiff != null) {
            if (thisDiff.operation == Operation.EQUAL) {
                ...
                */
                if (lastequality != null && ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.length()
                        < Diff_EditCost / 2) && ((
                        ...
                        if (both_types) {
                            ...
                            if (commonlength != 0) {
                                if (pointer.hasPrevious()) {
                                    thisDiff = pointer.previous();
                                    assert thisDiff.operation == Operation.EQUAL : "Previous diff should have been an" +
                                            " equality.";
                            ...
                            if (commonlength != 0) {
                                ...
                                thisDiff.text = text_insert.substring(
                                        text_insert.length() - commonlength) + thisDiff.text;
    ...
    public int match_main(String text, String pattern, int loc) {
        ...
        if (text.equals(pattern)) {
            ...
            // Nothing to match.
            return -1;
        } else if (loc + pattern.length() <= text.length() && text.substring(loc, loc + pattern.length()).equals(
                pattern)) {
    ...
    protected void patch_addContext(Patch patch, String text) {
        ...
        while (text.indexOf(pattern) != text.lastIndexOf(
                pattern) && pattern.length() < Match_MaxBits - Patch_Margin - Patch_Margin) {
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    ...
                    postpatch_text = postpatch_text.substring(0, char_count2) + aDiff.text + postpatch_text.substring(
                            char_count2);
                    break;
                case DELETE:
        ...
        for (Patch aPatch : patches) {
            ...
            if (start_loc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    if (text1.length() > this.Match_MaxBits && diff_levenshtein(
                            diffs) / (float) text1.length() > this.Patch_DeleteThreshold) {
